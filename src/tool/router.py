...
async def read_tool(tool: Tool = Depends(get_tool)):
    return tool

@router.post("/", response_model=ToolBase)
async def create_tool(tool: ToolBase, session: AsyncSession = Depends(get_session)):
...
async def delete_tool(
    tool: Tool = Depends(get_tool), session: AsyncSession = Depends(get_session)
):
